...
import (
	...
	"github.com/ray-project/kuberay/ray-operator/controllers/ray/batchscheduler/scheduler_plugins"
...
func ValidateBatchSchedulerConfig(logger logr.Logger, config Configuration) error {
	...
	if len(config.BatchScheduler) > 0 {
		...
		if config.BatchScheduler == volcano.GetPluginName() || config.BatchScheduler == yunikorn.GetPluginName() || config.BatchScheduler == scheduler_plugins.GetPluginName() {
